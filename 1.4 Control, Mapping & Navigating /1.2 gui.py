...
ScreenManager:
    ...
    # --- SCREEN BARU UNTUK SETUP NAVIGASI ---
    Screen:
        name: 'navigation_setup'
        on_enter: app.populate_map_list() # Panggil fungsi ini saat layar dibuka
        BoxLayout:
            orientation: 'vertical'
            padding: 40
            spacing: 20
            ...
            Spinner:
                ...
                size_hint_y: None
                height: '48dp'
                # values akan diisi oleh fungsi populate_map_list
            Button:
                text: 'Mulai Navigasi'
                font_size: '22sp'
                # Kirim teks dari Spinner sebagai argumen
    ...
    # --- SCREEN BARU UNTUK SESI NAVIGASI AKTIF ---
    Screen:
...
"""
    ...
    def start_navigation_session(self, map_name):
        if not map_name or map_name == 'Tidak ada peta ditemukan' or map_name == 'Pilih Peta...':
            self.update_status_label('navigation_setup', 'map_spinner', 'Pilih peta yang valid!')
            return
            
        status = self.manager.start_navigation(map_name)
        self.update_status_label('navigation_active', 'navigation_status_label', status)
    ...
    # --- FUNGSI SHUTDOWN & UPDATE LABEL (TIDAK BERUBAH) ---
    def on_stop(self):
