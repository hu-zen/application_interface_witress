...
# ROS Imports
# Kita letakkan di dalam try-except untuk memberi pesan error yang lebih baik
try:
    import rospy
    import rospkg
...
class RosManager:
    def __init__(self):
        ...
        self.status_callback = None # Akan diisi nanti oleh GUI
    ...
    def set_status_callback(self, callback):
        """GUI akan memanggil fungsi ini untuk mendaftarkan fungsi update statusnya."""
    ...
    def save_map(self, map_name):
        # (Fungsi ini tidak berubah, hanya mengirim pesan)
        if not map_name:
...
class MapSaverNode:
    def __init__(self):
        ...
        # Inisialisasi node ROS untuk listener ini
...
"""
    ...
    def on_start(self):
        """Hubungkan callback dari GUI ke manager setelah semuanya siap."""
...
if __name__ == '__main__':
    ...
    saver_thread = None
    try:
        # 1. Jalankan roscore dan tunggu
        print("[Main] Memulai roscore...")
        roscore_process = subprocess.Popen("roscore", preexec_fn=os.setsid)
        time.sleep(4) # Beri waktu 4 detik agar roscore benar-benar siap

        # 2. Setelah roscore siap, inisialisasi node ROS utama
        print("[Main] Inisialisasi Node ROS...")
        rospy.init_node('waiterbot_gui_main', anonymous=True)

        # 3. Jalankan "Program Khusus Penyimpan Peta" di thread terpisah
        ...
        # 4. Buat objek manager
        print("[Main] Membuat RosManager...")
        ros_manager = RosManager()
        
        # 5. Baru setelah semua siap, jalankan aplikasi Kivy
        ...
        # 6. Pastikan semua proses dimatikan saat program selesai
